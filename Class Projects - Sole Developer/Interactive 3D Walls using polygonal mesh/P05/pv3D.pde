...
String write() {return "("+x+","+y+","+z+")";};  // writes point coordinates in text window
   }
...
float angle(vec U, vec V,Boolean outerAlso) 
{
  float temp = acos(d(U,V)/n(V)/n(U)); 
  if(outerAlso)
  {
    vec tempCrossProd = N(U,V);
    if(tempCrossProd.z < 0)
    {
      temp = -temp;
    };
  }
