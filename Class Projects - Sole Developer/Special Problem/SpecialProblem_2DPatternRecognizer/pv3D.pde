...
class pt { float x=0,y=0,z=0 ,u=0,v=0,w=0; 
   pt () {}; 
     pt (float px, float py) {x = px; y = py;};
   pt (float px, float py, float pz) {x = px; y = py; z = pz; };
   pt (float px, float py, float pz, float pu, float pv) {x = px; y = py; z = pz; u = pu; v = pv;};
   pt (float px, float py, float pz, float pu, float pv, float pw) {x = px; y = py; z = pz; u = pu; v = pv; w = pw;};
...
void v(pt P, pt2 U) {vertex(P.x,P.y,P.z, U.x,U.y);};                                           // vertex for shading or drawing
