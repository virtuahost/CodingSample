...
void showArcThrough (pt2 A, pt2 B, pt2 C) {
 ...
 ArrayList<pt2> makeArcThrough (pt2 A, pt2 B, pt2 C) {
   ArrayList<pt2> ret = new ArrayList<pt2>();
  if (abs(dot(V2(A,B),R2(V2(A,C))))<0.01*d2(A,C)) {ret.add(A);ret.add(C); return ret;}
   pt2 O = CircumCenter ( A,  B,  C); 
   float r=d(O,A);
   vec2 OA=V2(O,A), OB=V2(O,B), OC=V2(O,C);
   float b = angle(OA,OB), c = angle(OA,OC); 
   ...
   ret.add(A); for (float t=0; t<1; t+=0.01) ret.add(R2(A,t*c,O)); ret.add(C); 
   return ret;
 }
